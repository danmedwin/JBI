...
def build_paragraph_page_map(doc, page_texts):
    ...
    # Find the footnotes boundary — stop mapping at the collected footnotes section
    # to prevent false matches from footnote text that echoes main content phrases
    footnotes_start = len(doc.paragraphs)
    for i, para in enumerate(doc.paragraphs):
        if para.text.strip() == "Footnotes":
            footnotes_start = i
            break

    para_to_page = {}
    min_page = 1
    # Maximum forward jump allowed (pages). A match that leaps further is likely
    # a false positive from footnotes or repeated text — skip it.
    MAX_JUMP = 15
    for i, para in enumerate(doc.paragraphs):
        if i >= footnotes_start:
            break
        ...
        for wp, ctext in collapsed:
            if wp < min_page:
                continue
            if sig in ctext:
                if wp - min_page > MAX_JUMP and min_page > 1:
                    continue  # skip suspicious long jump
                para_to_page[i] = wp
                min_page = wp
                found = True
                break
...
def is_section_header(para):
    s = get_style(para)
    return "Section Header" in s or "Heading" in s or "Hebrew Section Header" in s


def is_header_pair_start(para, doc):
    """Is this paragraph a Hebrew title paired with the SectionHeader that follows it?
    Catches cases where a Hebrew service title is styled as HebrewLiturgy instead of
    HebrewSectionHeader — the pair should never be split."""
    s = get_style(para)
    if "Hebrew" not in s:
        return False
    # Check if the next non-empty paragraph is a SectionHeader
    idx = para._p.getparent().index(para._p)
    siblings = list(para._p.getparent())
    for j in range(idx + 1, min(idx + 3, len(siblings))):
        from docx.oxml.ns import qn as _qn
        if siblings[j].tag == _qn('w:p'):
            pStyle = siblings[j].find('.//' + _qn('w:pStyle'))
            if pStyle is not None:
                val = pStyle.get(_qn('w:val'), '')
                if 'SectionHeader' in val or 'Heading' in val:
                    return True
            # Check if it has text (skip empties)
            texts = siblings[j].findall('.//' + _qn('w:t'))
            if any(t.text and t.text.strip() for t in texts):
                break
    return False
...
def adjust_insert_for_grouping(insert_idx, needs_split, doc, mapped_list, wp):
    ...
    para = doc.paragraphs[insert_idx]
    if is_section_header(para) or is_header_pair_start(para, doc):
    ...
    # Rule 2: Don't split a Hebrew header from its paired English header
    # (catches cases where insert_idx lands just before a Hebrew+English pair)
    if insert_idx + 1 < len(doc.paragraphs):
        next_para = doc.paragraphs[insert_idx + 1]
        if is_header_pair_start(next_para, doc):
            pos = page_paras.index(insert_idx) if insert_idx in page_paras else -1
            if pos > 0:
                insert_idx = page_paras[pos - 1]

    # Rule 3: Don't separate an attribution from its reading
...
def find_best_insert_point(wp, mapped_list, para_to_page, doc):
    ...
    for i in range(len(page_paras) - 1, 0, -1):
        ...
        if is_section_header(para):
            candidate_idx = i - 1
            # Don't strand another header or its Hebrew pair — keep scanning back
            while candidate_idx > 1 and (
                is_section_header(doc.paragraphs[page_paras[candidate_idx]])
                or is_header_pair_start(doc.paragraphs[page_paras[candidate_idx]], doc)
            ):
                candidate_idx -= 1
            if is_section_header(doc.paragraphs[page_paras[candidate_idx]]):
                continue
            if is_header_pair_start(doc.paragraphs[page_paras[candidate_idx]], doc):
                continue
