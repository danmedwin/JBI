...
const VERSION_STAMP = "v2026-07-08g build";
...
function parseElementsXml(text, layoutId) {
  ...
  while ((m = re.exec(match[1])) !== null) {
    ...
    if (layoutId === "kol_haneshamah") {
      ...
      if (["transliteration","translation","footnote","attribution","instructions"].includes(m[1])) {
  ...
  const SKIPPED_TYPES = new Set(["navigation", "header", "page_number", "bottom_section_name", "bottom_service_name"]);
...
const DOTTED_H_TYPES = new Set(["transliteration", "translation", "footnote", "instructions", "attribution"]);
...
function buildDocx(allPages, footnotes, fontSize, layoutId) {
  ...
  for (let pi=0; pi<allPages.length; pi++) {
    ...
    let ei=0, prevType="";
    for (const el of pg.elements||[]) {
      if (["navigation","header","page_number","bottom_section_name","bottom_service_name"].includes(el.type)) { ei++; continue; }
