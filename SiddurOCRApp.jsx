...
ITALIC PRESERVATION: When English text contains italic words or phrases, wrap the italic portions in <i>...</i> tags. Not for Hebrew or transliteration. Instructions are entirely italic — no tags needed.
...
- instructions: Italicized directions. Entirely italic — no <i> tags.
...
- attribution: Author/source credit IMMEDIATELY after the specific reading it belongs to. NEVER at the end of all readings.
- footnote: ALL text below a horizontal rule on the page. Includes COMMENTARY, NOTE, KAVANAH, DERASH sections and unlabeled notes. Use <i>...</i> for italic portions. If a footnote ends with an author's initials or name (e.g., "D.A.T.", "M.M.K.", "R.H."), keep them as part of the footnote — do NOT extract them as separate attribution elements.
...
<element type="translation" order="4">Second reading on the same page, also as one paragraph...</element>
<element type="attribution" order="5">Second Author</element>
<element type="translation" order="6">Third reading, if present...</element>
<element type="bottom_section_name" order="7">Amidah</element>
<element type="bottom_service_name" order="8">Kabbalat Shabbat</element>
<element type="page_number" order="9">127</element>
</elements>
...
5. Is there a horizontal rule on the page? ALL text below it must be footnote elements, not translation.
...
async function pdfToImages(file) {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    images.push({ pageNum: i, base64: dataUrl.split(",")[1], dataUrl });
  }
...
function reorderElements(elements) {
  ...
  function flush() {
    ...
    for (const el of buf) {
      if (el.type === "hebrew_liturgy" && cur.some(e => e.type === "hebrew_liturgy")) { groups.push(cur); cur = [el]; }
      else cur.push(el);
    }
    if (cur.length) groups.push(cur);
    for (const g of groups) { g.sort((a, b) => (LITURGICAL_ORDER[a.type] ?? 99) - (LITURGICAL_ORDER[b.type] ?? 99)); for (const el of g) result.push(el); }
    buf = [];
  }
...
function stripLatinDiacritics(t) { return t.replace(/[\u0304\u0306\u0307\u0323\u0330\u0331\u0332]/g, ""); }

function parseItalicSpans(raw) {
  const spans = []; const re = /<i>([\s\S]*?)<\/i>/g; let last = 0, m;
  while ((m = re.exec(raw)) !== null) {
    if (m.index > last) spans.push({ text: raw.slice(last, m.index), italic: false });
    spans.push({ text: m[1], italic: true }); last = re.lastIndex;
  }
  if (last < raw.length) spans.push({ text: raw.slice(last), italic: false });
  if (!spans.length) spans.push({ text: raw, italic: false });
  return spans;
}
...
function parseElementsXml(text, layoutId) {
  ...
  const els = []; const re = /<element\s+type="([^"]+)"\s+order="(\d+)">([\s\S]*?)<\/element>/g; let m;
...
async function analyzePageWithClaude(base64Image, systemPrompt, layoutId, attempt = 1, maxAttempts = 3) {
  ...
  try {
    ...
    if (attempt < maxAttempts) { await new Promise(r=>setTimeout(r,2000*attempt)); return analyzePageWithClaude(base64Image, systemPrompt, layoutId, attempt+1, maxAttempts); }
    throw new Error("Network error: "+err.message);
  }
  if (!response.ok) {
    const body = await response.text().catch(()=>"unknown");
    if (response.status===400 && isContentFilterError(body)) { if (attempt<maxAttempts) { await new Promise(r=>setTimeout(r,2000*attempt)); return analyzePageWithClaude(base64Image,systemPrompt,layoutId,attempt+1,maxAttempts); } let detail=""; try { const j=JSON.parse(body); detail=j.error?.message||""; } catch(_){} throw new Error("CONTENT_FILTER: Blocked after "+attempt+" attempts."+(detail?" Detail: "+detail:"")); }
    if (isRateLimitError(response.status,body)) { if (attempt<maxAttempts) { await new Promise(r=>setTimeout(r,5000*attempt)); return analyzePageWithClaude(base64Image,systemPrompt,layoutId,attempt+1,maxAttempts); } throw new Error("RATE_LIMIT: Exceeded after "+attempt+" attempts."); }
    if (response.status>=500 && attempt<maxAttempts) { await new Promise(r=>setTimeout(r,3000*attempt)); return analyzePageWithClaude(base64Image,systemPrompt,layoutId,attempt+1,maxAttempts); }
  ...
  try {
    ...
    if (attempt<maxAttempts) { await new Promise(r=>setTimeout(r,1000)); return analyzePageWithClaude(base64Image,systemPrompt,layoutId,attempt+1,maxAttempts); }
...
function postProcessKolHaneshamah(pages) {
  ...
  for (const p of pages) {
    ...
    const els = p.elements||[], hdrs = [];

    /* Split compound headers — both "NAME / NUMBER" and "NUMBER / NAME" patterns.
       Handle regular slash (/), fraction slash (⁄), division slash (∕), fullwidth slash (／) */
    const slashPat = '[/\u2044\u2215\uFF0F]';
    const nameThenNum = new RegExp('^(.+?)\\s*' + slashPat + '\\s*(\\d+)$');
    const numThenName = new RegExp('^(\\d+)\\s*' + slashPat + '\\s*(.+)$');
    for (let i=els.length-1; i>=0; i--) {
      const el = els[i], t = (el.text||"").trim();
      if (!(el.type==="section_header" || el.type==="page_number" || el.type==="header")) continue;

      /* Pattern 1: "NAME / NUMBER" (e.g., "MEDITATIONS / 9") */
      const nameFirst = t.match(nameThenNum);
      if (nameFirst) {
        els[i] = {type:"section_header", text:nameFirst[1].trim(), order:el.order};
        els.push({type:"page_number", text:nameFirst[2].trim(), order:9999});
        p.pageNumber = nameFirst[2].trim();
        continue;
      }

      /* Pattern 2: "NUMBER / NAME" (e.g., "128 / MEDITATIONS") */
      const numFirst = t.match(numThenName);
      if (numFirst) {
        els[i] = {type:"section_header", text:numFirst[2].trim(), order:el.order};
        continue;
      }
    }

    /* Fallback: clean up any section_headers that still have number/separator patterns */
    for (let i=0; i<els.length; i++) {
      if (els[i].type !== "section_header") continue;
      els[i].text = cleanHeaderText(els[i].text);
    }

    /* Fix compound pageNumber */
    if (p.pageNumber) {
      const pnMatch = (p.pageNumber+"").match(/^.+?\/\s*(\d+)$/);
      if (pnMatch) p.pageNumber = pnMatch[1].trim();
      const pnMatch2 = (p.pageNumber+"").match(/^(\d+)\s*\/\s*.+$/);
      if (pnMatch2) p.pageNumber = pnMatch2[1].trim();
    }

    /* Deduplicate only RUNNING HEADERS (the first section_header on each page).
       Content sub-headers like "TRADITIONAL VERSION" that appear later on the page are kept. */
    let foundFirstHeader = false;
    for (let i=0; i<els.length; i++) {
      if (els[i].type==="section_header") {
        if (!foundFirstHeader) {
    ...
    /* Promote bottom_section_name / bottom_service_name to section_header.
       CLEAN the text first (strip page numbers), then dedup against global set. */
    for (let i=els.length-1; i>=0; i--) {
      if (els[i].type==="bottom_section_name" && els[i].text.trim()) {
        const n=cleanHeaderText(els[i].text), nk=normalizeHeader(n);
        if (!seenHeaders.has(nk) && n) { seenHeaders.add(nk); hdrs.push({type:"section_header",text:n,order:-2}); }
        els.splice(i,1);
      } else if (els[i].type==="bottom_service_name" && els[i].text.trim()) {
        const n=cleanHeaderText(els[i].text), nk=normalizeHeader(n);
        if (!seenHeaders.has(nk) && n) { seenHeaders.add(nk); hdrs.push({type:"section_header",text:n,order:-1}); }
        els.splice(i,1);
      }
    }

    hdrs.sort((a,b)=>a.order-b.order);
    for (let h=hdrs.length-1; h>=0; h--) els.unshift(hdrs[h]);
    p.elements = els;
  }
  return pages;
}
...
function makeTextRuns(text, font, size, extra={}) {
  const { TextRun } = window.docx;
  return parseItalicSpans(text).map(s => new TextRun({ text: s.text, font, size, ...extra, ...(s.italic ? {italics:true} : {}) }));
}
...
function buildDocx(allPages, footnotes, fontSize, layoutId) {
  ...
  for (let pi=0; pi<allPages.length; pi++) {
    ...
    for (const el of pg.elements||[]) {
      if (["navigation","header","footnote","page_number","bottom_section_name","bottom_service_name"].includes(el.type)) { ei++; continue; }
      ...
      if (el.type==="section_header") {
        ...
        ch.push(new Paragraph({style:"Attribution",alignment:AlignmentType.RIGHT,spacing:{before:120,after:360},children:[new TextRun({text:el.text,font:T,size:is,italics:true})]}));
      } else if (el.type==="instructions") {
        /* Instructions: left-aligned, italic by default, but preserve <i> markup for mixed italic/non-italic */
        const hasItalicTags = /<i>/.test(el.text);
        if (hasItalicTags) {
          /* Mixed content: non-italic is default, <i> marks italic portions */
          ch.push(new Paragraph({style:"Instructions",spacing:{before:p12,after:p12},children:makeTextRuns(el.text,T,is)}));
        } else {
          /* Fully italic (no tags) */
          ch.push(new Paragraph({style:"Instructions",spacing:{before:p12,after:p12},children:[new TextRun({text:el.text,font:T,size:is,italics:true})]}));
        }
      }
      prevType=el.type;
      ei++;
  ...
  if (footnotes.length) {
    ...
    ch.push(new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:300},children:[new TextRun({text:"Footnotes",font:T,size:hs,bold:true})]}));
    ch.push(new Paragraph({border:{bottom:{style:BorderStyle.SINGLE,size:6,color:"999999"}},spacing:{after:200},children:[]}));
    const gr={}; for (const fn of footnotes) { if(!gr[fn.sourcePage]) gr[fn.sourcePage]=[]; gr[fn.sourcePage].push(fn.text); }
    for (const [pg,txts] of Object.entries(gr)) {
      ch.push(new Paragraph({spacing:{after:0},children:[]}));
      ch.push(new Paragraph({alignment:AlignmentType.LEFT,spacing:{before:200,after:120},children:[new TextRun({text:"Page "+pg,font:T,size:is,bold:true,color:"555555"})]}));
      for (const txt of txts) for (const fp of txt.split(/\n\s*\n/)) { const t=fp.replace(/\n/g," ").trim(); if(t) ch.push(new Paragraph({spacing:{after:120},children:makeTextRuns(t,T,fs)})); }
    }
  }
...
function TokenEstimate({ pageImages, selectedLayout, actualUsage }) {
  ...
  // Estimate
  const sysT = selectedLayout==="mishkan_tfilah"?2400:selectedLayout==="kol_haneshamah"?2200:1500;
  const perPage = sysT+2500+50;
  const totalIn = 2900+(perPage*numPages);
  ...
  const handleFile = useCallback(async (pdfFile) => {
    ...
    try {
      ...
      for (let p = 0; p < maxDetectPages; p++) {
        ...
        if (attempt.confidence > det.confidence) det = attempt;
      }
    ...
    try {
      ...
      for (let i=0; i<pageImages.length; i++) {
        ...
        try {
          let result;
          try {
            result = await analyzePageWithClaude(pageImages[i].base64, sysPrompt, selectedLayout);
          } catch(firstErr) {
            if (firstErr.message.startsWith("CONTENT_FILTER:")) {
              ...
              try {
                result = await analyzePageWithClaude(pageImages[i].base64, safePrompt, selectedLayout, 1, 2);
              } catch(_) { throw firstErr; /* safe mode also failed, throw original error */ }
            } else {
              throw firstErr;
            }
          }
          ...
          for (const fn of els.filter(e=>e.type==="footnote")) allFn.push({sourcePage:pn,text:fn.text});
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#f5f0e6 0%,#ebe4d4 50%,#e0d8c4 100%)",fontFamily:"'Georgia','Palatino Linotype',serif",padding:"0 16px"}}>
      <div style={{maxWidth:640,margin:"0 auto",paddingTop:48,paddingBottom:64}}>
        ...
        {status==="idle" && <div style={{marginTop:40,color:"#8a8070",fontSize:13,lineHeight:1.8}}>
          ...
          <div><strong style={{color:"#a07830"}}>5.</strong> A Word document is generated with named paragraph styles and collected footnotes.</div>
        </div>}
      </div>
    </div>
  );
