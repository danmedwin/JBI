...
const VERSION_STAMP = "v2026-07-13 build";
...
3. LEFT page, far-left margin: kavanot — meditational readings, poems, and alternative prayers, each usually with an italic title and a small-caps credit line (—AUTHOR, sometimes "(adapted)" or "(trans. Name)"). Sometimes a Hebrew piyyut with its English translation side by side.
4. RIGHT page, far-right margin: the running commentary, in smaller sans-serif type. Each note opens with its lemma (the phrase being commented on) in BOLD SMALL CAPS, usually followed by the Hebrew phrase. Commentary frequently OVERFLOWS the margin and continues as a full-width block across the bottom of the page — join a continuation to the note it belongs to. Torah-reading pages key notes to verses ("VERSE 1. TOOK NOTE OF SARAH...").
...
- hebrew_section_header: The red Hebrew title at the top of the right page (e.g. תפילת העמידה בלחש), or the large red Hebrew title of a section-opener.
...
- translation: English text of the main column. <i>...</i> for italic (congregational) spans. Keep verse numbers in Torah readings as plain digits (e.g. "1 Adonai took note of Sarah..."). Separate visually distinct blocks into separate elements.
- prayer_title: A heading within the main English column: the italic headings ("First B'rakhah: Our Ancestors", "Words of Welcome") AND the small-caps subheads ("U-NETANEH TOKEF—THE SACRED POWER OF THE DAY", "PRIVATE MEDITATION", "B'RAKHOT RECITED BY ONE CALLED UP TO THE TORAH"). Capture exactly as printed.
- instructions: ANY red rubric: procedural directions, conditional labels, speaker labels, variant labels, cross-references, and aliyah markers ("First Aliyah", "[Third Aliyah on Shabbat]", שני). IMPORTANT: the same rubric is usually printed TWICE on a spread — once on the English page and once above the Hebrew. Extract each distinct rubric ONCE, at the position before the content it governs.
...
async function splitPageAnalyze(base64Image, systemPrompt, layoutId) {
  ...
  const topResult = await analyzePageWithClaude(topHalf, systemPrompt + topNote, layoutId);
  const bottomResult = await analyzePageWithClaude(bottomHalf, systemPrompt + bottomNote, layoutId);
        ...
        try {
          ...
          try {
            ...
            if (firstErr.message.startsWith("CONTENT_FILTER:")) {
              ...
              try {
                result = await analyzePageWithClaude(pageImages[i].base64, safePrompt, selectedLayout, 1, 1);
              } catch(safeErr) {
                /* Safe mode also failed — try split-page mode */
                setStatusText("Page "+(i+1)+": safe mode failed, trying split-page mode\u2026");
                try {
                  result = await splitPageAnalyze(pageImages[i].base64, sysPrompt, selectedLayout);
